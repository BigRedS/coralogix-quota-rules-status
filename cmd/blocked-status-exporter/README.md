...
Computes the [blocked-status](../../README.md) report and pushes it to Coralogix
...
| `-ingest-key`     | `CX_INGEST_KEY`       | Send-Your-Data key for emitting |
| `-team`           | `CX_TEAM`             | value for the `team` label |
| `-cx-application` | `CX_APPLICATION_NAME` | `cx.application.name` (default `blocked-status`) |
...
```sh
go run ./cmd/blocked-status-exporter \
    -region eu2 -api-key "$MGMT_KEY" \
    -ingest-key "$INGEST_KEY" -team otel-demo

# see what would be sent, without sending:
go run ./cmd/blocked-status-exporter -region eu2 -api-key "$MGMT_KEY" -dry-run
```
...
GOOS=linux GOARCH=arm64 CGO_ENABLED=0 \
    go build -trimpath -ldflags "-s -w" -o bootstrap ./cmd/blocked-status-exporter
